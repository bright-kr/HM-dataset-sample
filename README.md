# H&M 데이터셋 샘플

<h2>1001개의 H&M 제품으로 구성된 샘플 데이터셋</h2>

![H&M products dataset header](https://github.com/bright-kr/HM-dataset-sample/blob/main/HM-dataset.png)

1000개 이상의 レコード로 구성된 H&M 제품 데이터셋 샘플입니다. 데이터셋는 <b>Bright Data API</b>를 사용하여 추출되었습니다.

<h2>H&M 제품 데이터셋에 포함된 일부 データポイント:</h2>

* ```category_tree```: 제품이 속한 カテゴリー 계층 구조  
* ```color```: 제품의 색상  
* ```country_code```: 제품의 위치 또는 가용성을 나타내는 국가 코드  
* ```county_of_origin```: 제품의 원산지 국가  
* ```currency```: 제품 가격이 표시된 통화  
* ```delivery```: 제품 배송에 관한 정보  
* ```description```: 제품에 대한 상세 설명  
* ```domain```: 제품이 등록된 도메인 또는 웹사이트  
* ```features```: 제품의 기능 또는 특성  
* ```final_price```: 제품의 현재 가격  
* ```image_count```: 제품과 연관된 전체 이미지 수  
* ```image_urls```: 제품 이미지로 연결되는 URL  
* ```in_stock```: 제품이 현재 재고가 있는지 여부를 나타냅니다  
* ```initial_price```: 제품의 원래 또는 초기 가격  
* ```main_image```: 제품과 연관된 메인 이미지의 URL 또는 식별자  
* ```manufacturer```: 제품의 제조사  
* ```offers```: 제품에 대한 다양한 오퍼 또는 패키지 정보  
* ```people_bought_together```: 현재 제품과 함께 흔히 구매되는 제품  
* ```product_name```: 제품명 또는 제목  
* ```reviews_count```: 제품의 전체 고객 리뷰 수  
* ```related_products```: 현재 제품과 관련된 다른 제품  
* ```seller_name```: 제품을 제공하는 판매자 이름  
* ```size```: 제품의 사이즈  
* ```top_reviews```: 제품에 대한 하이라이트 또는 상위 고객 리뷰  

그 외에도 더 많이 포함됩니다.

이는 410만 개 이상의 <b>4.1M records</b>를 포함하는 "H&M products"
데이터셋에서 파생된 샘플 サブセット입니다.

사용 가능한 데이터셋 파일 형식: <b>JSON, NDJSON, JSON Lines, CSV, or Parquet. Optionally, files can be compressed to .gz</b>.

데이터셋 전달 유형 옵션: <b>Email, API download, Webhook, Amazon S3, Google Cloud storage, Google Cloud PubSub, Microsoft Azure, Snowflake, SFTP</b>.

업데이트 빈도: <b>Once, Daily, Weekly, Monthly, Quarterly, or Custom basis</b>.

추출된 データポイント에 추가로 제공 가능한 Data enricH&Ment: <b>Based on request.</b>

<b>[전체 H&M 제품 데이터셋 받기](https://brightdata.co.kr/products/datasets/fashion/H&M)</b>.

<h2>H&M 제품 데이터셋의 활용 사례는 무엇입니까?</h2>

<h3>1. 가격 최적화</h3>
비교 가능한 H&M 제품 및 カテゴリー를 경쟁사와 비교 분석하여 가격 전략을 수립하고 동적 가격 모델을 구축합니다.

<h3>2. 제품 재고</h3>
H&M 제품의 재고 부족을 식별하고, 특정 아이템에 대한 수요 증가를 감지하며, 소비자 사이에서 떠오르는 트렌드를 파악합니다.

<h3>3. 시장 전략</h3>
H&M 데이터셋를 활용하여 시장 전략 분석을 수행하고, 핵심 트렌드와 소비자 선호를 식별합니다.

<h2>학술 연구자 및 NGO를 위한 Web스크레이핑 도구와 데이터셋 무료 이용</h2>

Bright Initiative는 다양한 환경 및 사회적 대의를 증진하는 주요 학술 기관 및 연구자, NGO 및 NPO에 Bright Data의 <b>[Web Scraper APIs](https://brightdata.co.kr/products/web-scraper)</b> 및 <b>[바로 사용 가능한 데이터셋](https://brightdata.co.kr/products/datasets)</b>에 대한 접근 권한을 제공합니다. <b>[여기](https://brightinitiative.com)</b>에서 신청서를 제출할 수 있습니다.